...
void case1(int capa, int wide, int prev, string options[]){
    system("cls");
    cout << "Acceptable inputs: down arrow, up arrow or Enter." << endl;
    cout << "Current category: ";
    if(prev==0){cout << "<not created/chosen>" << endl;}
    else if(options[prev-1]=="<empty>"){cout << "<not created/chosen>" << endl;}
    else cout << options[prev-1] << endl;
...
void New_Category(int CHOICE, string options[]){
    string name;
    string line_out;
    cout << "Wyglada na to, ze tworzysz nowy katalog. Jak chcialbys go nazwac? ";
    getline(cin, name);
    fstream file_opt;
    file_opt.open("Options.txt", ios::out | ios::app);
    if(!file_opt.good()){
...
int Interface(vector<Word> &YE, int prev, string options[]){
    ...
    switch(getch()){
        ...
        case 13: {system("cls");
            cout << "Loading..." << endl;
            if(choice==0) {system("cls"); if(prev==0){cout << "Wybierz najpierw kategorie!"; system("pause"); goto menu;}; Revise(YE); goto menu;}
            if(choice==1) {system("cls"); if(prev==0){cout << "Wybierz najpierw kategorie!"; system("pause"); goto menu;}; Add(YE, files[(2*prev)-2], files[(2*prev)-1]); goto menu;}
            if(choice==2) {system("cls"); if(prev==0){cout << "Wybierz najpierw kategorie!"; system("pause"); goto menu;}; Illustrate(YE); goto menu;}
...
int main()
{
    ...
    what << prev;
    what.close();
    cout << "Dzieki za skorzystanie z programu! Nacisnij cokolwiek, aby kontynuowac" << endl; system("pause");
    return 0;
}
//ideas
...
//display that user have not added any words when trying to revise or view them
